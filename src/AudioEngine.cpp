...
size_t AudioDecoder::readSamples(AudioBuffer& buffer, size_t numSamples,
    ...
    // ═══════════════════════════════════════════════════════════════
    // ✅ CRITICAL: Convert DFF → DSF for DAC compatibility
    // ═══════════════════════════════════════════════════════════════
    // DFF format: MSB First + Big Endian
    // DSF format: LSB First + Little Endian
    // Conversion requires: Byte Swap + Bit Reversal on each byte
    // ═══════════════════════════════════════════════════════════════
    ...
    if (m_trackInfo.codec.find("msbf") != std::string::npos) {
        ...
        // Lookup table for fast bit reversal (256 entries)
        ...
        for (size_t i = 0; i < numWords; i++) {
            ...
            // Step 1: Byte swap (Big Endian → Little Endian)
            // Convert: ABCD → DCBA
            word = ((word & 0x000000FF) << 24) |  // D → A position
                   ((word & 0x0000FF00) << 8)  |  // C → B position
                   ((word & 0x00FF0000) >> 8)  |  // B → C position
                   ((word & 0xFF000000) >> 24);   // A → D position
            
            // Step 2: Bit reversal (MSB → LSB) on each byte
            // This reverses the bit order within each byte
            uint8_t* bytes = reinterpret_cast<uint8_t*>(&word);
            bytes[0] = bitReverseTable[bytes[0]];  // Reverse bits in byte 0
            bytes[1] = bitReverseTable[bytes[1]];  // Reverse bits in byte 1
            bytes[2] = bitReverseTable[bytes[2]];  // Reverse bits in byte 2
            bytes[3] = bitReverseTable[bytes[3]];  // Reverse bits in byte 3
            
            // Store converted word back
            data32[i] = word;
        }
        
        // Log conversion once
        static bool conversionLogged = false;
        if (!conversionLogged) {
            std::cout << "[AudioDecoder] 🔄 DFF → DSF conversion applied:" << std::endl;
            std::cout << "[AudioDecoder]    - Byte swap: Big Endian → Little Endian" << std::endl;
            std::cout << "[AudioDecoder]    - Bit reversal: MSB First → LSB First" << std::endl;
            std::cout << "[AudioDecoder]    ✅ DFF converted to DSF format" << std::endl;
